...
def slice_single_set(h5_dataset, y_range, x_range, out):
    ...
    begin = time.time()
    h5_dataset.read_direct(out, source_sel=np.s_[y_begin:y_end, x_begin:x_end])
    sliced_set = out
    end = time.time() - begin
    print("Finished slicing. It took {} seconds.".format(end))
    with open("/home/ubuntu/waveq/src/logs/read_direct_experiment.txt", "a") as f:
...
def slice_sets(vars_list):
    ...
    y_range = [0,2918]
    x_range = [0,1600]
    out = np.empty((2918, 1600), dtype=np.float64)
    for var in vars_list:
        single_sliced = slice_single_set(var, y_range, x_range, out)
