"""
Proof of concept module to automate reading of h5 data sets into RDD format into Spark
-- ensure buoy_links directory is on worker machines as well as the driver.
"""
...
def get_s3_buoy(csv_line):
    ...
    # Log issues that arise
    f = open("log.txt", "a")
    f.write("This is the h5_file: {}".format(h5_file))


    # Get relevant info from h5 file
    metric = lines[1]
    f.write("This is the metric: {}".format(metric))
    f.close()
    data_set = h5_file[metric]
...
def get_time_index_buoy(start_year):
    ...
    for offset in range(1, 4):
        s3_endpoint = "s3://wpto-pds-us-wave/v1.0.0/virtual_buoy/US_virtual_buoy_" + str(start_year + offset) + ".h5"
        s3 = s3fs.S3FileSystem(anon=True)
        s3_file = s3.open(s3_endpoint, "rb")
        h5_file = h5py.File(s3_file, "r")
        time_h5 = h5_file['time_index'][:]
...
def run_four_year_block(start_year, end_year):
    ...
    time_df = get_time_index_buoy(start_year)
    ...
    s3_endpoint = "s3://wpto-pds-us-wave/v1.0.0/virtual_buoy/US_virtual_buoy_1979.h5"
    ...
    # This is a proof of concept work so recall that you will want to specify
    # a multi table or single table logic when joining metrics.
